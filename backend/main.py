...
class BacktestRequest(BaseModel):
    ...
    days_before: int = 21
    range_call_pct: float = 4.95
    range_put_pct: float = 4.95
...
def backtest_strangle_yfinance(df, days_before=21, range_call_pct=4.95, range_put_pct=4.95, premium_pct=1.71,
    ...
    while (cur_year < end.year) or (cur_year == end.year and cur_month <= end.month):
        ...
        upper_strike = opening_price * (1 + range_call_pct / 100)
        lower_strike = opening_price * (1 - range_put_pct / 100)
        ...
        days_passed = len(pd.bdate_range(start=open_date, end=close_date)) - 1
...
async def run_backtest(request: BacktestRequest):
    try:
        ...
        results = backtest_strangle_yfinance(
            df, request.days_before, request.range_call_pct, request.range_put_pct, request.premium_pct,
